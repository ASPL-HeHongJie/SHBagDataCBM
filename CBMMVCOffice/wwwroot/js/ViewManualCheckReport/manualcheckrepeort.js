var vm = new Vue({
    ...
    methods: {
        search() {
            ...
            var ipaddress = "/api/CheckData/GetManualCheckData";
            axios.post(
                ipaddress,
                data,
        ...
        OpenReport(reporttype) {
            ...
            if (this.TimeDifference(this.BtnDateTime, new Date()) >= 5) {
                ...
                if (reporttype == "FR") {
                    ...
                    let reportIPAddress = this.reportServerIpAndPort.split(':')[0]
                    setTimeout(() => {
                        this.dialogloading = true;
                        $("#Loading").css("display", "block");
                        $("#div_iframe").css("display", "none");
                        $("#report").attr('src', 'http://' + this.reportServerIpAndPort +'/FRReportForm.aspx?ID=' + this.FRCheckData[0].hisID + '&ManufacturerName=' + this.FRCheckData[0].brandName + '&Address=' + reportIPAddress);
                    }, 0);
                } else {
                    this.StationsData.filter(el => el.Name == this.VOSCheckData[0].stationName).map((res) => {
                        this.reportIPAddress = res.IPAddress
                    ...
                    setTimeout(() => {
                        ...
                        $("#Loading").css("display", "block");
                        $("#div_iframe").css("display", "none");
                        $("#report").attr('src', 'http://' + this.reportServerIpAndPort +'/VOSReportForm.aspx?ID=' + this.VOSCheckData[0].hisID + '&ManufacturerName=' + this.VOSCheckData[0].brandName + '&Address=' + reportIPAddress);
                    }, 0)
                }
            }
        },
